...
const mainDomains = new Set([
	...
	'challenge.developers.cloudflare.com',
...
export default {
	...
	async fetch(request, ctx) {
		try {
			...
			const re = new RegExp("target=clash&url=https");
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				else if (reqPath == `/sub/${userID}`) {
					const subconf = GenSub(userID, hostName );
					return await new Response(`${subconf}`, {
							status: 200,
							headers: {
				...
				else if (reqPath == `/${userID}`) {
					const vConfig = getConfig(userID, hostName );
					return await new Response(`${vConfig}`, {
							status: 200,
							headers: {
								"Content-Type": "text/plain;charset=utf-8",
							}
...
/**
 * 
 * @param { ArrayBuffer} vBuffer 
 * @param {string} userID 
 * @returns 
 */
function processVHeader(
	vBuffer,
	userID
) {
	...
	let isUDP = false;
	if (stringify(new Uint8Array(vBuffer.slice(1, 17))) === userID) {
		isValidUser = true;
	}
	if (!isValidUser) {
		return {
 ...
 * @param {ArrayBuffer} vResponseHeader 
...
function getConfig(userID, hostName) {
	...
	const clash_link = `https://${hostName}/sub?target=clash&url=${encodeURIComponent(`https://${hostName}/sub/${userID}?format=clash`)}&insert=false&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
	return `
################################################################
sub clash
---------------------------------------------------------------
...
function GenSub(userID, hostName) {
